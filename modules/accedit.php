...
if ($row0 = mysql_fetch_assoc($res)) $rip  = $row0['ip'];
...
if  (($_POST['id'] == 51) AND ($ra_email == '')) {
   if  (!eregi("^[a-zA-Z0-9\._-]+@[a-zA-Z0-9\._-]+\.[a-zA-Z]{2,4}$", $_POST['new_email'])) {
       ...
       .$txt[20].'</b></td></tr><tr><td height="45" align="center" valign="middle"  class="ErrTab"><b>'
  ...
  mysql_query('insert into `mail` (`random`, `account`, `email`, `mode`) values ("'.$mail_code.'", '.$ra_id.', "'.$_POST['new_email'].'", 4)');
  echo $k_query.'<br>';
  mysql_query($k_query);
  $http_mail = 'http://'.$_SERVER['HTTP_HOST'].$_SERVER['PHP_SELF'].'?modul=mail&id='.$mail_code;
    ...
    // отсылка по почте
	$mail_event = 'standart';
  if  ($_POST['id'] == 1) $mail_body = $txt[61];
...
if ((($_POST['id'] == 1) OR ($_POST['id'] == 2)) AND ($ra_email != '')) {
  $mail_code = generate(40);
  $k_connect = mysql_connect($k_ip, $k_userdb, $k_pw);
  mysql_select_db($k_db, $k_connect);
  mysql_query("SET NAMES '".$encoding."'");
  mysql_query('delete from `mail` where (`account` = '.$ra_id.')');
  mysql_query('insert into `mail` (`random`, `account`, `email`, `mode`) values ("'.$mail_code.'", '.$ra_id.', "'.$ra_email.'", '.$_POST['id'].')');
  echo $k_query.'<br>';
  mysql_query($k_query);
  $http_mail = 'http://'.$_SERVER['HTTP_HOST'].$_SERVER['PHP_SELF'].'?modul=mail&id='.$mail_code;
...
if ($_POST['id'] == 3) {//type select
                echo '<b>'.$txt[8].'</b><br><br><img src="images/other/box_wow.jpg">&nbsp;<img src="images/other/box_tbc.jpg">&nbsp;<img src="images/other/box_wotlk.jpg"><br><br><form method="POST"><input name="modul" value="accedit" type=hidden>';
				echo '<select name=type>';
 				for ($i = 0; $i <= 2; $i++) {
   					echo '<option value='.($i+1);
					if ($i == $row['expansion']) echo ' selected>';
					else echo '>';
					echo getExpansion($i).'</option>';
				}
           		echo '</select>&nbsp;<input type=submit value="'.$txt[11].'"></form><br><br><hr><div align="center"><A href="index.php">'.$txt[12].'</a></div>';
                //ReturnMainForm(10);
				return;
				}
if ($_POST['type'] >= 1) { //type accept
				mysql_query("UPDATE `account` set `expansion` = ".($_POST['type']-1)." WHERE `id` = ".$_SESSION['user_id']);
				echo '<br><img src="images/other/ok.png"><br><br><img src="images/yes.png"> '.$txt[16].'<b> - '.getExpansion($_POST['type']-1).'</b><br><br><hr><div align="center"><A href="index.php">'.$txt[12].'</a></div>';
                ReturnMainForm(60);
 				return;
				}
if ($_POST['id'] == 5) { //unlock
				if (($ra_email == '') OR ($ra_last_ip == $_SESSION['ip'])) {
  			        mysql_query("UPDATE `account` set `locked` = 0 WHERE `id` = ".$_SESSION['user_id'].";");
				    echo '<br><img src="images/other/ok.png"><br><br><img src="images/yes.png"> <b>'.$txt[17].
  					     '</b><br><br><hr><div align="center"><A href="index.php">'.$txt[12].'</a></div>';
                    ReturnMainForm(60);
				    return; }
				else {
  $mail_code = generate(40);
  ...
  mysql_query("SET NAMES '".$encoding."'");
  mysql_query('delete from `mail` where (`account` = '.$ra_id.')');
  mysql_query('insert into `mail` (`random`, `account`, `email`, `mode`) values ("'.$mail_code.'", '.$ra_id.', "'.$ra_email.'", 3)');
  echo $k_query.'<br>';
  mysql_query($k_query);
  $http_mail = 'http://'.$_SERVER['HTTP_HOST'].$_SERVER['PHP_SELF'].'?modul=mail&id='.$mail_code;
    // отсылка по почте
...
if ($_POST['id'] == 4) { //lock
				...
				echo '<br><img src="images/other/ok.png"><br><br><img src="images/yes.png"> <b>'.$txt[18].'!</b><br><br><hr><div align="center"><A href="index.php">'.$txt[12].'</a></div>';
                ReturnMainForm(60);
				return;
				}
echo '<table width="500" border="0" cellspacing="0" cellpadding="0"><tr><td height="25" align="center" valign="middle" class="ErrTitle">
<b>'.$txt[20].'</b></td></tr><tr><td height="45" align="center" valign="middle"  class="ErrTab"><b>'.$txt[19].'</b></td></tr></table>
<br><br>';
?> 
<br>
