...
2 - заменить емайл
3 - unlock IP
4 - установить емайл вместо пустого(неверного)
*/
if ($_POST['id'] == 13) {
 if  (eregi("^[a-zA-Z0-9\._-]+@[a-zA-Z0-9\._-]+\.[a-zA-Z]{2,4}$", $_POST['new_email'])) {
    ...
    mysql_query("SET NAMES '$encoding'");
    mysql_query("UPDATE `account` set `email` = '".$_POST['new_email']."' WHERE `id` = ".$row['account']);
	...
	echo '<b><a href="?modul=mail&id='.$_GET['id'].'">'.$txt[115].'</a></b>
<br><br>
<div align="center"><A href="index.php">'.$txt[12].'</a></div>
';
  }
return;
    }
if  ($row['mode'] == 1) {
    ...
    mysql_select_db($r_db, $r_connect);
    mysql_query("SET NAMES '$encoding'");
    $rrr =mysql_query("select `username` from `account`  WHERE `id` = ".$row['account']." limit 1");
    $r3 = mysql_fetch_assoc($rrr);
    $new_pass = strtoupper(generate(10));
    $new_hash = SHA1(strtoupper($r3['username']).':'.strtoupper($new_pass));
    mysql_query("UPDATE `account` set `sha_pass_hash` = '".$new_hash."' WHERE `id` = ".$row['account']);
	$mail_event = 'standart';
	$mail_body = $txt[117];
	...
	$log_old_data  = $r3['sha_pass_hash']; // сохраняем старый хеш в логе, вдруг придется восстанавливать!?
	require('include/log.php');
	$k_connect = mysql_connect($k_ip, $k_userdb, $k_pw);
    mysql_select_db($k_db, $k_connect);
    mysql_query("SET NAMES '".$encoding."'");
...
if  ($row['mode'] == 4) {
    ...
    mysql_select_db($r_db, $r_connect);
    mysql_query("SET NAMES '$encoding'");
    mysql_query("UPDATE `account` set `email` = '".$row['email']."' WHERE `id` = ".$row['account']);
    echo '<br><img src="images/other/ok.png"><br><br><img src="images/yes.png"> <b>'.$txt[114].'</b><br>';
    $log_account   = $row['account'];
    $log_character =  0;
	$log_mode      =  5;
...
else echo '<br><table width="500" border="0" cellspacing="0" cellpadding="0"><tr><td height="25" align="center" valign="middle" class="ErrTitle">
